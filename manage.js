...
function createProposalItem(proposal) {
    ...
    let shootingDate = '調整中';
    if (proposal.year && proposal.month && proposal.day) {
        shootingDate = `${proposal.year}年${proposal.month}月${proposal.day}日`;
        
        // 時刻の表示を柔軟に処理
        if (proposal.startTime || proposal.endTime) {
            let timeRange = '';
            if (proposal.startTime && proposal.endTime) {
                timeRange = `${proposal.startTime}〜${proposal.endTime}`;
            } else if (proposal.startTime) {
                timeRange = `${proposal.startTime}〜（終了時刻調整中）`;
            } else if (proposal.endTime) {
                timeRange = `（開始時刻調整中）〜${proposal.endTime}`;
            }
            shootingDate += ' ' + timeRange;
        }
    }
...
function editProposal(id) {
    ...
    document.getElementById('editStartTime').value = proposal.startTime || '';
    document.getElementById('editEndTime').value = proposal.endTime || '';
...
function saveEditedProposal() {
    ...
    if (!modelName || !location || selectedTypes.length === 0 || selectedPurposes.length === 0) {
        alert('モデル名、撮影場所、撮影タイプ、利用目的は必須項目です。');
        return;
    }
